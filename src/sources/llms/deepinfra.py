...
class DeepInfraChat:
    """
    A class to encapsulate interaction with the DeepInfra `llama3.3:latest` model using OpenAI-compatible API.
    """
    def __init__(self, model: str = "meta-llama/Llama-3.3-70B-Instruct", api_key: str = "$DEEPINFRA_TOKEN", base_url: str = "https://api.deepinfra.com/v1/openai"):
        """
        Initialize the DeepInfraChat class with a specified model.

        Args:
            model (str): The name of the model to interact with.
            api_key (str): The API key for authentication.
            base_url (str): The base URL of the DeepInfra API.
        """
        self.model = model
...
if __name__ == "__main__":
    ...
    response = deepinfra_chat.generate_response(prompt)
    print(response)
    
    # Generate responses for a batch of prompts
    batch_prompts = ["Hello", "How are you?", "What is AI?"]
    print("\n\nBatch Responses:")
    batch_responses = deepinfra_chat.generate_batch(batch_prompts)
