...
#     """
#     python bird_evaluation/evaluation.py \
#     --predicted_sql_path bird/process/bird/PPL_DEV_BIRD.JSON-9_SHOT_Euclidean_mask_1534/llamaapi_3.3_output_eval.json \
...
# if __name__ == "__main__":
#     # Example usage
#     candidate_sql = "SELECT CAST(SUM(CASE WHEN T1.isNonFoilOnly = 1 THEN 1 ELSE 0 END) AS REAL) * 100 / COUNT(T1.id)  FROM cards AS T1  JOIN set_translations AS T2 ON T1.setCode = T2.setCode  WHERE T2.language = 'Japanese';"
...
#     db_path = "bird/bird/database/card_games/card_games.sqlite"
...
#     predicted_sql: SELECT CAST(SUM(CASE WHEN T1.isNonFoilOnly = 1 THEN 1 ELSE 0 END) AS REAL) * 100 / COUNT(T1.id)  FROM cards AS T1  JOIN set_translations AS T2 ON T1.setCode = T2.setCode  WHERE T2.language = 'Japanese';
...
# db_path: bird/bird/database/card_games/card_games.sqlite
# sql_dialect: SQLite
# SQLite error: no such column: T1.isNonFoilOnly