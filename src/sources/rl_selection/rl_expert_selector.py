...
class ExpertSelectionEnv:
    ...
    def step(self, action: int):
        ...
        from evaluation import evaluate_partial
        partial_scores = evaluate_partial(gold_sql, pred_sql, self.db_path, self.kmaps)
        # 權重組合
        reward = (
            0.4 * partial_scores['select']['f1'] +
            0.3 * partial_scores['where']['f1'] +
            0.1 * partial_scores['group(no Having)']['f1'] +
            0.1 * partial_scores['order']['f1'] +
            0.1 * partial_scores['IUEN']['f1']
        )
        # diversity bonus
